...
void solut() {
    ...
    vi temp;
    LL tot = 0, j = 1;
    for (int i = n - 1; i >= 0; i--, j *= 2) tot += (s[i] - '0') * j;
    std::string t;
    std::cin >> t;
    int m = t.size();
    for (int i = m - 1; i >= 0; i--) {
    ...
    vi rres;
    while (res) rres.push_back(res & 1), res >>= 1;
    while (rres.size() < n - 1) rres.push_back(0);
    reverse(all(rres));
    for (int i : rres) std::cout << i;
    std::cout << '\n';
}
