...
int main() {
    ...
    while (t--) {
        ...
        // dp //
        vvi f(n + 1, vi(n + 1));    // f[i][j]: 前 i 个还有 j 个没有匹配时的方案数 //
        vvi g(n + 1, vi(n + 1));    // g[i][j]: 前 i 个还有 j 个没有匹配时的答案 //
        f[0][0] = 1;
        for (int i = 1; i <= n; i++) {
            for (int j = 0; j <= s[i]; j++) {
                // 不匹配 //
                if (j > 0) {
                    Add(f[i][j], f[i - 1][j - 1]);
                    Add(g[i][j], sub(g[i - 1][j - 1], mul(f[i - 1][j - 1], x[i])));
                }
                // 匹配 //
                Add(f[i][j], mul(f[i - 1][j + 1], j + 1));
                Add(g[i][j], add(mul(g[i - 1][j + 1], j + 1), mul(f[i - 1][j + 1], x[i], j + 1)));
            }
        }
        std::cout << g[n][0] << '\n';
    }
