...
int main() {
    ...
    while (t--) {
        ...
        auto solve = [&](vi& low, vi& upp) -> int {
            int n = low.size();
            int ans = 0, res = low.size();
            int flag = 0;    // flag = true: 脱离下界 //
        ...
        if (low.size() == upp.size()) {
            ...
            for (int dig = low.size(); dig < upp.size(); dig++) Max(ans, dig * 3);
            low = {1};
            low.resize(upp.size());
            reverse(all(upp));
        }
        Max(ans, solve(low, upp));
        std::cout << ans << '\n';
    }
    return 0;
}
