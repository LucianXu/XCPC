...
void solut() {
    ...
    auto check = [&](LL x) -> bool {
        vi cnt(n - m + 3);
        auto add = [&](int l, int r) { cnt[l]++, cnt[r + 1]--; };
        for (int i = 1; i <= n; i++) {
            int le = std::max(i - n + m, 1);
            int ri = std::min(i, m);
            if (a[i] >= x) {
                add(1, n - m + 1);
            } else if (a[i] + b[ri] < x) {
                ...
                LL k = (x - a[i] - c + d - 1) / d + 1;
                add(i - ri + 1, i - (int) k + 1);
            }
        }
        for (int i = 1; i <= n - m + 2; i++) cnt[i] += cnt[i - 1];
        assert(cnt[n - m + 2] == 0);
        return *max_element(all(cnt)) >= k;
    };
