...
int main() {
    ...
    int ans = 0;
    // 01 背包? //
    static std::vector<std::array<int, 2>> f(
        k + 1, std::array{-inf, -inf});    // f[i][op]: 容量为 i, op 为 0/1 表示是否都满 //
    f[0][0] = 0;
    for (int i = 1; i <= n; i++) {
        std::vector g = f;
        for (int v = k; v >= 0; v--) {
            int w = a[i][0];
            if (v + w <= k) {
                Max(g[v + w][0], f[v][0] + a[i][w]);
                Max(g[v + w][1], f[v][1] + a[i][w]);
            }
            for (int w = 1; w < a[i][0]; w++) {
                if (v + w <= k) {
                    Max(g[v + w][1], f[v][0] + a[i][w]);
                }
            }
        }
        f = g;
