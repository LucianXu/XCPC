...
int main() {
    ...
    while (t--) {
        ...
        auto power = [&](auto power, int a, int n, int p) -> int {
            int ans = 1;
            while (n) {
        ...
        for (int i = 1; i <= n; i++) {
            for (int j = 1; j <= 2 and i + j <= n; j++) {
                cnt[(LL) a[i + j] * power(power, a[i], p - 2, p) % p]++;
            }
        }
        for (auto [p, ccnt] : cnt)
            if (ccnt * 8 >= n) v.push_back(p);
        std::sort(all(v));
        v.erase(unique(all(v)), v.end());
        for (auto q : v) {
            std::map<int, int> mp;
            int qq = power(power, q, p - 2, p), maxx = 0;
            for (int i = 1; i <= n; i++) {
                mp[a[i]] = mp[(LL) a[i] * qq % p] + 1;
                Max(maxx, mp[a[i]]);
            }
            Max(ans, maxx);
        }

        std::cout << (2 * ans >= n ? ans : -1) << '\n';
    }
    return 0;
}
