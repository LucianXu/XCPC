...
int main() {
    ...
    auto fact = [&](auto fact, int x) -> void {
        for (int i = 2; i * i <= x; i++) {
            if (x % i != 0) continue;
            int tmp = 0;
            while (x % i == 0) x /= i, tmp++;
        ...
        if (x != 1) v.push_back(x), mapper[x]++;
    };

    auto power = [&](auto power, int a, int n, int mod) -> int {
        int ans = 1;
        while (n) {
    ...
    for (int i = 1; i <= n; i++) fact(fact, a[i]);
    int ans = 1;
    for (auto [p, cnt] : mapper) {
        ans = (LL) ans * power(power, p, std::min(cnt, n - cnt), mod) % mod;
    }
    std::cout << ans << '\n';
