...
int main() {
    ...
    while (t--) {
        ...
        static bool a[505][505];
        static int up[505][505], down[505][505];
        for (int i = 1; i <= n; i++) {
            for (int j = 1; j <= m; j++) {
                up[i][j] = 0;
                down[i][j] = n + 1;
        ...
        for (int _ = 1; _ <= n * m; _++) {
            ...
            std::cin >> x >> y;
            int lu = 1, ld = n;
            for (int l = y; l >= 1 and !a[x][l]; l--) {
                Max(lu, up[x][l] + 1);
                Min(ld, down[x][l] - 1);
                int ru = lu, rd = ld;
                for (int r = y; r <= m and !a[x][r]; r++) {
                    Max(ru, up[x][r] + 1);
                    Min(rd, down[x][r] - 1);
                    ans -= 1LL * (rd - x + 1) * (x - ru + 1);
                }
            }
            std::cout << ans << '\n';
            for (int i = x - 1; i >= 1 and !a[i][y]; i--) {
                down[i][y] = x;
            }
            for (int i = x + 1; i <= n and !a[i][y]; i++) {
                up[i][y] = x;
            }
            a[x][y] = true;
        }
