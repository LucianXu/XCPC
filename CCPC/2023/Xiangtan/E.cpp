...
int main() {
    ...
    while (t--) {
        ...
        std::vector<Z> fac(1000010), fac_inv(1000010);
        fac[0] = Z(1);
        for (int i = 1; i <= 1000000; i++) fac[i] = fac[i - 1] * i;
        fac_inv[1000000] = inv(fac[1000000]);
        for (int i = 999999; i >= 0; i--) fac_inv[i] = fac_inv[i + 1] * (i + 1);

        Z ans = 0;

        auto C = [&](Z n, Z m) -> Z {
            if (n < m) return 0;
            return fac[n] * fac_inv[n - m] * fac_inv[m];
        };
