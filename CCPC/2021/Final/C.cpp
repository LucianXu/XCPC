...
int main() {
    ...
    while (t--) {
        int n;
        std::cin >> n;
        vi a(n + 1), fac(n + 1), fac_inv(n + 1);

        fac[0] = 1;
        for (int i = 1; i <= n; i++) fac[i] = mul(fac[i - 1], i);
        fac_inv[n] = pow(fac[n], mod - 2);
        for (int i = n - 1; i >= 0; i--) fac_inv[i] = mul(fac_inv[i + 1], i + 1);

        for (int i = 1; i < n; i++) std::cin >> a[i];

        auto C = [&](int n, int m) -> int { return mul(fac[n], fac_inv[m], fac_inv[n - m]); };

        int res = 1;
        for (int i = n - 1; i >= 1; i--) Mul(res, C(a[i + 1] + 1, a[i]));
        std::cout << res << '\n';
    }
    return 0;
}
